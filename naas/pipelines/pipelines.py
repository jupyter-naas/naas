...
class ExecutionContext:
    ...
    def __init__(self, output_dir : str = 'pipeline_executions'):
        ...
        self.output_path = os.path.join(self.output_dir ,f'{self.timestamp.strftime("%Y-%m-%d_%H-%M-%S")}-{self.execution_id}')
...
class Step():
    ...
    def get_all_steps(self, depth:int = 0):
        ...
        for s in self.next_steps:
            tasks += s.get_all_steps(depth+2 if isinstance(self, ParallelStep) else depth+1)
        if self.on_error != None and (len(self.on_error.steps) > 0 or len(self.on_error.next_steps) > 0):
    ...
    def show_dag(self, depth:int = 0 ,net: Network = None, height: str = "1000px", width: str = "100%"):
        ...
        if self.node_image:
            net.add_node(self.name, x=depth, y=0, level=depth, color=color, label=node_label, mass=self.node_mass, image=self.node_image, shape=self.node_shape)
        else:
            net.add_node(self.name, x=depth, y=0, level=depth, color=color, label=node_label, mass=self.node_mass, shape=self.node_shape)
...
class Pipeline(Step):
    ...
    def monitor_status(self):
        ...
        with out:
            ...
            with progress:
                ...
                task_ids = {}
                for step in steps:
                        name = step.get('name') if 'ParallelStep' not in step.get('name') else 'ParallelStep'
                        task_ids[step.get('name')] = progress.add_task(step.get('name'),
                                          name=name,
                ...
                while self.status != StepStatus.COMPLETED or last_run < 2 :
                    ...
                    for step in steps:
                        progress.update(task_id=task_ids[step.get('name')], status=f'[{status_style[step.get("status")]}] {step.get("status").name}')
    ...
    def run(self, style: Literal['diagram', 'progess'] = 'diagram', monitor:bool = True, outputs_path=''):
        ...
        if monitor is True:
            if style == 'progress':
                self.monitors.append(threading.Thread(target=self.monitor_status, daemon=True))
            else:
                self.monitors.append(threading.Thread(target=self.monitor_diagram, daemon=True))
...
class NotebookStep(Step):
    ...
    def run(self, ctx:ExecutionContext):
        ...
        self.status = StepStatus.RUNNING
        try:
            sanitized_name = "".join(c for c in self.name if c.isalnum() or c in ('_')).rstrip()

            
            out_file = os.path.join(ctx.output_path, f'{sanitized_name}.{os.path.basename(self.notebook_path)}')
        ...
        if self.status == StepStatus.COMPLETED:
            self.run_next_steps(ctx)
        elif self.on_error != None and (len(self.on_error.steps) > 0 or len(self.on_error.next_steps) > 0):
            self.on_error.run(ctx)